...
public class OktaConnectorTests extends AbstractConnectorTests {
    ...
    private static OktaConnector conn;
    ...
    public void changeUserPassword() {
        ...
        try {
            ...
            String username = UUID.randomUUID().toString();
            Attribute password = AttributeBuilder.buildPassword(new GuardedString("Password123".toCharArray()));
            Attribute newPassword = AttributeBuilder.buildPassword(new GuardedString("123Password".toCharArray()));
            ...
            User user = conn.getClient().getUser(updated.getUidValue());
            UserCredentials userCredentials = conn.getClient().instantiate(UserCredentials.class);
            PasswordCredential passwordCredentials = conn.getClient().instantiate(PasswordCredential.class);
            passwordCredentials.setValue(newPassword.getValue().get(0).toString().toCharArray());
